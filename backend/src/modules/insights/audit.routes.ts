...
auditRouter.post("/", requireAuth, requireRole(Role.admin, Role.asset_manager), async (req, res, next) => {
  try {
    ...
    if (!name || !start_date || !end_date) {
      throw new AppError("VALIDATION_ERROR", 400, "Missing required fields: name, start_date, end_date");
    }
    ...
    try {
      ...
      if (err.message === "END_DATE_BEFORE_START_DATE" || err.message === "INVALID_DATES") {
        throw new AppError("UNPROCESSABLE", 422, err.message);
      }
      throw err;
    }
...
auditRouter.post("/:id/auditors", requireAuth, requireRole(Role.admin, Role.asset_manager), async (req, res, next) => {
  try {
    ...
    if (!auditor_user_id) throw new AppError("VALIDATION_ERROR", 400, "Missing auditor_user_id");
    const actor = req.user!;
    try {
      ...
      if (err.message === "AUDITOR_ALREADY_ASSIGNED") {
        throw new AppError("CONFLICT", 409, err.message);
      }
      throw err;
    }
  } catch (error) { next(error); }
...
auditRouter.get("/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (!cycle) throw new AppError("NOT_FOUND", 404, "Audit cycle not found");
      res.status(200).json(cycle);
    } catch (err: any) {
      if (err.message === "UNAUTHORIZED_CYCLE_ACCESS") {
        throw new AppError("FORBIDDEN", 403, err.message);
      }
      throw err;
    }
  } catch (error) { next(error); }
...
auditRouter.get("/:id/discrepancy-report", requireAuth, async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (err.message === "CYCLE_NOT_FOUND") throw new AppError("NOT_FOUND", 404, err.message);
      if (err.message === "UNAUTHORIZED_CYCLE_ACCESS") throw new AppError("FORBIDDEN", 403, err.message);
      throw err;
    }
  } catch (error) { next(error); }
});
...
auditRouter.post("/:id/close", requireAuth, requireRole(Role.admin, Role.asset_manager), async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (err.message === "CYCLE_NOT_FOUND") throw new AppError("NOT_FOUND", 404, err.message);
      if (err.message === "ALREADY_CLOSED") throw new AppError("UNPROCESSABLE", 422, err.message);
      throw err;
    }
  } catch (error) { next(error); }
});
...
auditItemRouter.patch("/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    if (!result || !validResults.includes(result)) {
      throw new AppError("VALIDATION_ERROR", 400, `result must be one of: ${validResults.join(", ")}`);
    }
    const actor = req.user!;
    try {
      ...
      if (err.message === "ITEM_NOT_FOUND") throw new AppError("NOT_FOUND", 404, err.message);
      if (err.message === "CYCLE_CLOSED") throw new AppError("UNPROCESSABLE", 422, err.message);
      if (err.message === "UNAUTHORIZED") throw new AppError("FORBIDDEN", 403, err.message);
      throw err;
    }
  } catch (error) { next(error); }
});
